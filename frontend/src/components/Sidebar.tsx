...
import { LayoutDashboard, ArrowLeftRight, PieChart, Sparkles, TrendingUp, LogOut, UserCircle } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside style={{ position: "fixed", top: 0, left: 0, width: "240px", height: "100vh", backgroundColor: "#111118", borderRight: "1px solid #1e1e2e", display: "flex", flexDirection: "column", zIndex: 50 }}>
      <div style={{ padding: "24px", borderBottom: "1px solid #1e1e2e" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <span style={{ color: "white", fontWeight: 700, fontSize: "20px" }}>Ledgr<span style={{ color: "#10b981" }}>.ai</span></span>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: "16px", display: "flex", flexDirection: "column", gap: "4px" }}>
        {NAV.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 12px", borderRadius: "8px", fontSize: "14px", fontWeight: 500, textDecoration: "none", color: active ? "#34d399" : "#9ca3af", backgroundColor: active ? "rgba(16,185,129,0.08)" : "transparent", border: active ? "1px solid rgba(16,185,129,0.2)" : "1px solid transparent" }}>
              <Icon size={18} />{label}
            </Link>
          );
        })}
      ...
      <div style={{ borderTop: "1px solid #1e1e2e" }}>
        <Link href="/profile" style={{ display: "flex", alignItems: "center", gap: "12px", padding: "16px", textDecoration: "none", borderBottom: "1px solid #1e1e2e", backgroundColor: pathname === "/profile" ? "rgba(16,185,129,0.05)" : "transparent" }}>
          {user?.imageUrl ? (
